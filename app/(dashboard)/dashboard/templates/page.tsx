...
export default function TemplatesPage() {
  ...
  // Generate share link (only if content changed)
  const generateShareLink = async () => {
    if (!cardRef.current) return;

    const currentContent = getContentHash();

    // If content hasn't changed and we have a share URL, don't regenerate
    if (currentContent === lastSharedContent && shareUrl) {
      return;
    }
  ...
  return (
    <div className="h-full">
      ...
      {/* Share Drawer */}
      <div
        className={`fixed inset-0 z-50 transition-opacity duration-300 ${showShareDrawer ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
        onClick={() => setShowShareDrawer(false)}
      >
        <div className="absolute inset-0 bg-black/60" />
      </div>
      <div
        className={`fixed top-0 right-0 h-full w-[380px] bg-[#0f0f17] border-l border-purple-500/20 z-50 transform transition-transform duration-300 ease-out overflow-y-auto ${showShareDrawer ? "translate-x-0" : "translate-x-full"}`}
      >
        {/* Drawer Header */}
        ...
        <div className="p-4 space-y-6">
          {/* Copy Image */}
          <div>
            <label className="text-sm text-gray-400 mb-2 block">Copy to Clipboard</label>
            <button
            ...
            >
              {copiedImage ? <Check className="w-5 h-5" /> : <Copy className="w-5 h-5" />}
          ...
          <div>
            ...
            {!shareUrl ? (
              <button
                ...
                disabled={isSharing}
                className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl font-medium hover:opacity-90 transition disabled:opacity-50"
              >
                {isSharing ? <Loader2 className="w-5 h-5 animate-spin" /> : <Link2 className="w-5 h-5" />}
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 p-3 bg-white/5 rounded-xl border border-white/10">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm font-medium transition flex-shrink-0 ${
                      copiedLink
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center gap-1 p-3 bg-white/5 hover:bg-[#1DA1F2]/20 border border-white/10 hover:border-[#1DA1F2]/30 rounded-xl transition group"
                >
                  <svg className="w-6 h-6 text-gray-400 group-hover:text-white" viewBox="0 0 24 24" fill="currentColor">
                ...
                >
                  <svg className="w-6 h-6 text-gray-400 group-hover:text-[#1877F2]" viewBox="0 0 24 24" fill="currentColor">
                ...
                >
                  <svg className="w-6 h-6 text-gray-400 group-hover:text-[#0A66C2]" viewBox="0 0 24 24" fill="currentColor">
                ...
                >
                  <Send className="w-6 h-6 text-gray-400 group-hover:text-[#0088cc]" />
                ...
                >
                  <svg className="w-6 h-6 text-gray-400 group-hover:text-[#FF4500]" viewBox="0 0 24 24" fill="currentColor">
                ...
                >
                  <svg className="w-6 h-6 text-gray-400 group-hover:text-[#E60023]" viewBox="0 0 24 24" fill="currentColor">
              ...
              <button
                ...
                className="w-full mt-3 flex items-center justify-center gap-2 px-4 py-3 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-gray-400 hover:text-white rounded-xl font-medium transition"
              >
